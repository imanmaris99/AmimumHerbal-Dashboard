    ...
    <div className="space-y-8 pb-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">Monitor live customer orders from the backend.</p>
        </div>
              ...
              {isLoading ? (
                ...
                filteredOrders.map((order) => (
                  <TableRow key={order.id} className="group hover:bg-gray-50/50 transition-colors border-gray-50">
                    ...
                    <TableCell className="text-sm font-medium text-gray-700">{order.customer_name || 'Unknown Customer'}</TableCell>
                    <TableCell className="text-sm text-gray-600 capitalize">{order.delivery_type}</TableCell>
                    <TableCell className="font-bold text-gray-900">Rp {Number(order.total_price || 0).toLocaleString('id-ID')}</TableCell>
                    <TableCell>
                      <Badge variant="secondary" className="bg-orange-50 text-orange-600 hover:bg-orange-100 border-none font-bold text-[10px] py-0.5 rounded-lg px-2 uppercase">
                        {order.status}
                      </Badge>
                    </TableCell>
      ...
      <Card className="border-none shadow-sm rounded-3xl bg-orange-50 border-orange-100 overflow-hidden p-8">
        <h3 className="text-lg font-bold text-orange-900">Live integration status</h3>
        <p className="text-orange-700 text-sm mt-2 leading-relaxed">
          Orders page is now connected to the backend live endpoint with basic status filtering and frontend search. Detail flow can be deepened in the next batch.
        </p>
        <p className="mt-2 text-[10px] font-bold text-orange-800 uppercase tracking-wider">
          Source: GET /admin/orders
        </p>
      </Card>
    </div>
  );
