...
import { FileText, FolderTree, PlusCircle, Search, LayoutList } from 'lucide-react';
  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        ...
        <Card className="border-none shadow-sm rounded-3xl"><CardContent className="p-6"><div className="flex items-center justify-between"><div className="p-3 rounded-2xl bg-violet-50 text-violet-600"><PlusCircle className="w-5 h-5" /></div><span className="text-[10px] font-bold uppercase tracking-wider text-gray-400">Create</span></div><p className="text-sm font-medium text-gray-500 mt-4">Category Action</p><p className="text-2xl font-bold text-gray-900 mt-1">Active</p><p className="text-[11px] text-gray-400 mt-2">POST /categories/post sudah tersambung</p></CardContent></Card>
      </div>
          ...
          <CardContent className="px-8 pb-8">
            <form onSubmit={submitCategory} className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                <textarea id="category-description" value={categoryForm.description} onChange={(e) => setCategoryForm((prev) => ({ ...prev, description: e.target.value }))} placeholder="Deskripsi kategori konten" className="min-h-[120px] rounded-2xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-700 outline-none w-full" required />
              </div>
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-300">
          <div>
            ...
            <ul className="list-disc pl-5 space-y-1">
              ...
              <li>form create category tidak submit saat field kosong</li>
            </ul>
          </div>
          <div>
